...
#TEXT_DICT=loadGloveModel('../data/glove.6B.200d.txt')
#import make_glove_dict
...
def bow_averaged_vectors(headlines, bodies):
    ...
    for i, (headline, body) in tqdm(enumerate(zip(headlines, bodies))):
        ...
        if len(vec_wsh)>0:
            X_hi = np.mean(np.array(vec_wsh), axis=0)/np.linalg.norm(np.array(vec_wsh))
        else:
            X_hi = np.random.randn(200,)
        if len(vec_wsb)>0:
            X_bi = np.mean(np.array(vec_wsb), axis=0)/np.linalg.norm(np.array(vec_wsb))
        else:
